...
ap.add_argument("-t", "--threshold", type=float, default=0.01,help="minimum threshold for pixel-wise mask segmentation")
ap.add_argument("-u", "--use-gpu", type=bool, default=0,help="boolean indicating if CUDA GPU should be used")
ap.add_argument("-e", "--iter", type=int, default=5,help="# of GrabCut iterations (larger value => slower runtime)")
...
(Ht, Wid) = image.shape[:2]
# image = imutils.resize(image, width=600)
image1 = cv2.resize(image, (580, 650),  interpolation=cv2.INTER_LINEAR)
# image1 = cv2.resize(image, (Ht-100, Wid-150),interpolation = cv2.INTER_NEAREST)
cv2.imshow("Input", image1)
# construct a blob from the input image and then perform a
# forward pass of the Mask R-CNN, giving us (1) the bounding box
# coordinates of the objects in the image along with (2) the
# pixel-wise segmentation for each specific object
blob = cv2.dnn.blobFromImage(cv2.cvtColor(image1,cv2.COLOR_BGR2GRAY) , crop=False)
net.setInput(blob)
...
for i in range(0, boxes.shape[2]):
    ...
    if confidence >= args["confidence"] :
        ...
        mask = cv2.resize(mask, (boxW, boxH),interpolation=cv2.INTER_LANCZOS4)
        ...
        rectangle = (0, 0, W - 5, H - 5)
        (gcMask, bgModel, fgModel) = cv2.grabCut(image1, gcMask,rectangle, bgModel, fgModel, iterCount=args["iter"],mode=cv2.GC_INIT_WITH_MASK)
        ...
        background = image1 - output
        background[np.where((background > [0, 0, 0]).all(axis=2))] = [255, 255, 255]
        final = background + output
        cv2.imwrite('Img_'+ str(i) + '_final.jpg', final)
